...
class Vertex(Widget):
    ...
    def touch_collide_point(self, x, y):
        # checks whether coordinates collide with this vortex
...
class ShowPassword(LoginScreen):
    ...
    def draw(self):
        with self.canvas:
            Color(0, .9, .9)
            for i, coord in enumerate(POSITIONS):
                position = (WIDTH * coord[0] - AOI_RADIUS / 2, HEIGHT * coord[1] - AOI_RADIUS / 2)
                colored = False
    ...
    def _get_arrow_pos(self):
        # returns the position of the arrow (which shows the direction of the pattern)
